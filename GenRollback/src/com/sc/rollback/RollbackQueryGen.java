...
import com.org.conn.ConnectionProvider;
...
public class RollbackQueryGen {
	...
	public DeleteScript createRollbackForInsert(InsertScript ins,Connection con)
	{
		...
		try {
			ins.setTableName(ConnectionProvider.p.getProperty("schema")+"."+ins.getTableName().substring(ins.getTableName().indexOf(".")+1));
	...
	public ArrayList<InsertScript> createRollbackForDelete(DeleteScript des,Connection con)
	{
		...
		des.setTableName(ConnectionProvider.p.getProperty("schema")+"."+des.getTableName().substring(des.getTableName().indexOf(".")+1));
	...
	public ArrayList<UpdateScript> createRollbackForUpdate(UpdateScript ups,Connection con)
	{
		...
		try {
			...
			ups.setTableName(ConnectionProvider.p.getProperty("schema")+"."+ups.getTableName().substring(ups.getTableName().indexOf(".")+1));
